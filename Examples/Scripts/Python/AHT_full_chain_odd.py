...
parser.add_argument(
    "--aht-max-division", help="Maksymalny poziom podzialu AHT", type=int, default=8
)
parser.add_argument(
    "--aht-threshold", help="Wartosc threshold dla AHT", type=int, default=3
)
parser.add_argument(
    "--aht-noise", help="Wartosc noiseThreshold dla AHT", type=int, default=12
)
...
if args.edm4hep:
    ...
    from acts.examples.edm4hep import PodioReader  #from acts.examples.podio import PodioReader
...
if args.reco:
    aht_config = acts.examples.AdaptiveHoughTransformSeeder.Config()

    aht_config.maxDivision = args.aht_max_division
    aht_config.threshold = args.aht_threshold
    aht_config.noiseThreshold = args.aht_noise
    
    aht_config.phiMinBinSize = 3.14 / (2.0 * 257.0)
    aht_config.qOverPtMinBinSize = 1.1 / (2.0 * 257.0)
    aht_config.qOverPtMin = 1.1
    aht_config.doSecondPhase = True
    aht_config.zMinBinSize = 1 * u.mm
    aht_config.cotThetaMinBinSize = 0.1
    aht_config.deduplicate = True
    ...
    addSeeding(
        ...
        outputDirRoot=outputDir if args.output_root else None,
        outputDirCsv=outputDir if args.output_csv else None,
        seedingAlgorithm=SeedingAlgorithm.AdaptiveHoughTransform,
        adaptiveHoughTransformConfig=aht_config,
    )
